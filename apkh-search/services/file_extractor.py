...
# Try importing pytesseract — graceful fallback if Tesseract not installed
try:
    import pytesseract
    
    # On Windows, try to auto-detect the default Tesseract installation folder 
    # so users don't have to restart their terminal to load the PATH variable.
    if os.name == 'nt':
        default_tesseract_path = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
        # Check if the binary exists at the default installation path
        if os.path.exists(default_tesseract_path):
            pytesseract.pytesseract.tesseract_cmd = default_tesseract_path
            
    # Test if tesseract works by asking for version
    pytesseract.get_tesseract_version()
    TESSERACT_AVAILABLE = True
except Exception:
    TESSERACT_AVAILABLE = False
    logger.warning("Tesseract binary not found or pytesseract not available. OCR will be skipped.")
